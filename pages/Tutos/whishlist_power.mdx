...
> Télécharger et activer l’app (**$**15,99 / month).

> Activer l’app sur votre branche du theme.

> Créer le template '**wishlist**' dans les pages.
...
2. Créer le fichier mm-wishlist.js dans le dossier assets
``` JavaScript
...
3. Créer le fichier mm-wishlist.css dans le dossier assets
``` CSS
/* ----------------- Wishlist content  */
...
2. Créer la section MM-Wishlist-Products dans le dossier sections
``` Liquid
...
Créer le snippet JS.liquid utilisé dans la section 
``` Liquid
{% render "mm-wishlist-card-js" %}
```
...
3. Créer le snippet du bouton (ajout à la wishlist ou suppression).
``` liquid{% comment %}
...
{% endif %}
```
Exemple section Main-Product (page produit): 
``` Liquid
...
Créer le snippet mm-icon-wishlist-close
``` Liquid
...
Ne pas oublier de les ajouter au snippet qui gère les icones mm-icon.liquid (si présent).
``` Liquid
when 'wishlist-close'
