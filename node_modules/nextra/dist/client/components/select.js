...
const Select = (t0) => {
  ...
  if ($[0] !== className) {
    t1 = (t22) => {
      ...
      return cn("x:h-7 x:rounded-md x:px-2 x:text-xs x:font-medium x:transition-colors", open ? "x:bg-gray-200 x:text-gray-900 x:dark:bg-primary-100/10 x:dark:text-gray-50" : hover ? "x:bg-gray-100 x:text-gray-900 x:dark:bg-primary-100/5 x:dark:text-gray-50" : "x:text-gray-600 x:dark:text-gray-400", focus && "x:nextra-focus", className);
    };
    $[0] = className;
    $[1] = t1;
  } else {
...
function _temp2(option) {
  return /* @__PURE__ */ jsx(ListboxOption, { value: option.id, as: Fragment, children: (t0) => {
    ...
    return /* @__PURE__ */ jsxs("li", { className: cn(focus_0 ? "x:bg-primary-50 x:text-primary-600 x:dark:bg-primary-500/10" : "x:text-gray-800 x:dark:text-gray-100", "x:cursor-pointer x:whitespace-nowrap x:py-1.5 x:px-3", "x:transition-colors", selected && "x:flex x:items-center x:justify-between x:gap-3"), children: [
      option.name,
      selected && /* @__PURE__ */ jsx(CheckIcon, { height: "16" })
    ] });
  } }, option.id);
}
export {
